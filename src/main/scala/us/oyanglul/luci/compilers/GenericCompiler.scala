...
trait GenericCompiler[E[_]] extends LowPriorityGenericCompiler[E] {
  ...
  )(implicit ev0: Monad[E], ev1: Lazy[Compiler[F1, E]], ev2: Compiler[EitherK[F2, F3, ?], E]) =
    prg.foldMap(compileN[F1, F2, F3].compile)

  def compile[F1[_], F2[_], F3[_], R1, R2 <: HList, A, Eff[A] <: EitherK[F2, F3, A]](
      prg: FreeT[EitherK[F1, EitherK[F2, F3, ?], ?], E, A]
  )(implicit ev0: Monad[E], ev1: Lazy[Compiler.Aux[F1, E, R1]], ev2: Compiler.Aux[EitherK[F2, F3, ?], E, R2]) = {
    prg
  ...
  implicit def compileN[A[_], B[_], C[_]](implicit ia: Lazy[Compiler[A, E]], ib: Compiler[EitherK[B, C, ?], E]) =
